...
class ComRewardlabsControllerCart extends ComKoowaControllerModel
{
    /**
     * Sales Receipt Service
     *
     * @var ComRewardlabsAccountingServiceInventoryInterface
     */
    ...
    {
        $config->append(array(
            'model'             => 'com://site/rewardlabs.model.carts',
            'inventory_service' => 'com://site/rewardlabs.accounting.inventory',
        ));
    ...
    protected function _actionAdd(KControllerContextInterface $context)
    {
        ...
        if (count($cart))
        {
            ...
            if (!in_array($data->ItemRef, $cartItems))
            {
                ...
                $item = $this->getObject('com://site/rewardlabs.model.cartitems')->create($cartItemData);
                $item->save();
            }
        }
    ...
    protected function _actionCheckout(KControllerContextInterface $context)
    {
        ...
        $controller = $this->getObject('com://site/rewardlabs.controller.order');
        $controller->add($data);
    ...
    protected function _actionDeleteitem(KControllerContextInterface $context)
    {
        $data  = $context->request->data;
        $ids   = $data->id;
        $items = $this->getObject('com://admin/rewardlabs.model.cartitems')->id($ids)->fetch();
