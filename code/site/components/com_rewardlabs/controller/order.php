...
class ComRewardlabsControllerOrder extends ComKoowaControllerModel
{
    ...
    protected function _initialize(KObjectConfig $config)
    {
        $config->append(array(
            'behaviors' => array(
                'com://admin/dragonpay.controller.behavior.cancellable' => array(
                ...
                'processable',
                'shippable',
                'com://admin/xend.controller.behavior.shippable',
                'rewardable',
                'accountable',
                'chargeable',
                'referrerrewardable',
    ...
    {
        // Inventory service
        $identifier = $this->getIdentifier('com://site/rewardlabs.accounting.inventory');
        $service    = $this->getObject($identifier);
        if (!($service instanceof ComRewardlabsAccountingInventoryInterface))
        {
            throw new UnexpectedValueException(
                "Service $identifier does not implement ComRewardlabsAccountingInventoryInterface"
            );
        }

        $translator = $this->getObject('translator');
        $data       = $context->request->data;
        $account    = $this->getObject('com://site/rewardlabs.model.accounts')->id($data->account_id)->fetch();
        $cart       = $this->getObject('com://site/rewardlabs.model.carts')->id($data->cart_id)->fetch();
        $error      = false;
        $result     = false;
        ...
        {
            $order_status    = ComRewardlabsModelEntityOrder::STATUS_COMPLETED;
            $invoice_status  = 'paid';
            $payment_method  = 'cash';
            $shipping_method = 'na';
    ...
    protected function _validateDelivered(KControllerContextInterface $context)
    {
        ...
        try
        {
            ...
            {
                if ($entity->order_status <> ComRewardlabsModelEntityOrder::STATUS_SHIPPED) {
    ...
    protected function _validateCompleted(KControllerContextInterface $context)
    {
        ...
        try
        {
            ...
            foreach ($entities as $entity)
            {
                if ($entity->order_status <> ComRewardlabsModelEntityOrder::STATUS_DELIVERED) {
    ...
    protected function _validateCancelorder(KControllerContextInterface $context)
    {
        ...
        try
        {
            ...
            foreach ($orders as $order)
            {
                ...
                if (!in_array($order->order_status, array(ComRewardlabsModelEntityOrder::STATUS_PAYMENT, ComRewardlabsModelEntityOrder::STATUS_PENDING))) {
    ...
    protected function _actionAdd(KControllerContextInterface $context)
    {
        ...
        $account    = $this->getObject('com:rewardlabs.model.accounts')->id($user->getId())->fetch();
        $translator = $this->getObject('translator');
        $data       = $context->request->data;
        $cart       = $this->getObject('com://site/rewardlabs.model.carts')->id($data->cart_id)->fetch();
        ...
        if ($order->save())
        {
            ...
            $order = $this->getObject('com://site/rewardlabs.model.orders')->id($order->id)->fetch();
    ...
    {
        $context->request->data->order_status = ComRewardlabsModelEntityOrder::STATUS_PROCESSING;
    ...
    {
        $context->request->data->order_status       = ComRewardlabsModelEntityOrder::STATUS_SHIPPED;
    ...
    protected function _actionMarkdelivered(KControllerContextInterface $context)
    {
        $context->getRequest()->setData([
            'order_status' => ComRewardlabsModelEntityOrder::STATUS_DELIVERED
        ]);
    ...
    protected function _actionCancelorder(KControllerContextInterface $context)
    {
        // Mark as Paid
        $context->getRequest()->setData([
            'order_status' => ComRewardlabsModelEntityOrder::STATUS_CANCELLED
        ]);
    ...
    protected function _activateReward(KModelEntityInterface $order)
    {
        ...
        if ($order->order_status != ComRewardlabsModelEntityOrder::STATUS_COMPLETED) {
        ...
        foreach ($rewards as $reward) {
            $this->getObject('com:rewardlabs.controller.reward')->id($reward->id)->activate();
        }
    }
}
